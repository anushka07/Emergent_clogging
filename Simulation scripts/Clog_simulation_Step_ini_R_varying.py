...
def setup(use_petsc=0,kernel_language='Python',outdir='./_output_Rstepvary_Step_input',solver_type='classic',disable_output=False):
    ...
    x = pyclaw.Dimension(-0.0,1.0,2500,name='x')    
    ...
    phim0 = 0.65
    rc = 0.5
    phin =0.45  # Initial value of particle fraction in the whole channel:  (i) phin = 0.35 corresponds to the unclogged solution
                #                                                           (ii) phin = 0.45 corresponds to the clogged solution      
    ...
    state.q[0,:] = phi_s                                        # Steady state for phi = 0.2
    state.q[0,0:100] = phin                                     # Step input of higher particle fraction
    state.q[0,:] = state.q[0,:]*(state.aux[2,:])**2             # q = phi* R^2    
